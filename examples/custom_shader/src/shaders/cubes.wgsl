...
fn vs_main(vertex: Vertex, cube: Cube) -> Output {
    ...
    //convert to tangent space to calculate lights in same coordinate space
    //as normal map sample
    let tangent = normalize(normal_matrix * vertex.tangent);
    let normal = normalize(normal_matrix * vertex.normal);
    let bitangent = cross(tangent, normal);
    //for shifting everything into tangent space
    ...
    out.tangent_light_pos = tbn * uniforms.light_pos.xyz;

    return out;
}

//cube properties, for now they are just const
...
fn fs_main(in: Output) -> @location(0) vec4<f32> {
    ...
    var normal = textureSample(normal_texture, sky_sampler, in.uv).xyz;
    ...
    let reflection_color = textureSample(sky_texture, sky_sampler, reflection_dir);
    let refraction_dir = refract(dir_to_camera, normal, REFRACTION_INDEX);
    let refraction_color = textureSample(sky_texture, sky_sampler, refraction_dir);
    let final_reflect_color = mix(reflection_color, refraction_color, 0.5);

    //don't account accurately for transparency for now
